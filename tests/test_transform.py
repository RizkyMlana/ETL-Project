...
# Melakukan test convert dari USD ke IDR
...
def test_clean_data():
    data = [
        {"Title": "T-shirt Cihuuuy", "Price": "$199.99", "Rating": "5", "Colors": "3", "Size": "M", "Gender": "Men" },
    ...
    # Memastikan kolom Price sudah dalam IDR
    assert cleaned_data[0]["Price"] == 3199840
    ...
    assert all(isinstance(float(item["Rating"]), float) for item in cleaned_data)
